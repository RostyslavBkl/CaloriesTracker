...
const processGraphQLErrors = (
  ...
  if (!errors?.length)
    return context === "login" ? "Login failed" : "Registration failed";
...
const validateAuthPayload = (
  ...
  if (payload.success === false) {
    ...
    if (isEmailExistsError(msg)) return "Email already exists";
    if (isPasswordError(msg))
      return payload.message || "Password must be at least 6 characters";
    return payload.message || "Registration failed";
  }
  ...
  action$.pipe(
    ...
    switchMap((action: ReturnType<typeof loginStart>) => {
      ...
      return from(loginApi(email, password)).pipe(
        ...
        catchError((err) =>
          of(loginFailure(err?.message || "Network error. Please try again."))
        )
      );
    })
  );
  ...
  action$.pipe(
    ...
    switchMap((action: ReturnType<typeof registerStart>) =>
      from(registerApi(action.payload.email, action.payload.password)).pipe(
        map((result) => {
          if (result?.errors?.length) {
            return registerFailure(
              processGraphQLErrors(result.errors, "register")
            );
          }
        ...
        catchError((err) =>
          of(
            registerFailure(err?.message || "Network error. Please try again.")
          )
        )
      )
    )
